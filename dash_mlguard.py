"""dash_mlguard -- lint for ML training pipelines.
...
class Report:
    ...
    def summary(self) -> str:
        ...
        return f"dash_mlguard: {c} critical, {w} warning, {i} info"

    def __str__(self) -> str:
        if not self.findings:
            return "dash_mlguard: no issues found."
    ...
    def to_pdf(
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                "to_pdf() requires fpdf2. Install with `pip install dash-mlguard[pdf]` "
...
def _render_metrics_block(
    ...
    return (
        '<h2>Performance: before vs after dash_mlguard</h2>'
        f'<div class="metrics">{col("Before (naive)", before)}'
        f'{col("After (dash_mlguard-cleaned)", after)}</div>'
        f"{delta_html}"
    )
...
def _render_html(
    ...
    *,
    ...
    meta_bits.append(f"dash_mlguard v{__version__}")
    ...
    return (
        ...
        f"<h2>Findings</h2>"
        f"{findings_html}"
        f"<footer>Generated by dash_mlguard. One call: <code>dash_mlguard.check(X_train, y_train, X_test, y_test)</code></footer>"
        f"</body></html>"
    )
...
def _render_pdf(
    ...
    *,
    ...
    meta.append(f"dash_mlguard v{__version__}")
    ...
    if metrics_before or metrics_after:
        pdf.set_text_color(*_PDF_RGB["fg"])
        pdf.set_font("Helvetica", "B", 11)
        pdf.cell(0, 6, "Performance: before vs after dash_mlguard", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(1)
        ...
        for ci, (heading, data) in enumerate(
            [("Before (naive)", metrics_before), ("After (dash_mlguard-cleaned)", metrics_after)]
        ):
