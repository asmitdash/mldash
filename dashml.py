"""dashml -- lint for ML training pipelines.
...
class Report:
    ...
    def summary(self) -> str:
        ...
        return f"dashml: {c} critical, {w} warning, {i} info"

    def __str__(self) -> str:
        if not self.findings:
            return "dashml: no issues found."
    ...
    def to_pdf(
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                "to_pdf() requires fpdf2. Install with `pip install dashml[pdf]` "
...
def _render_metrics_block(
    ...
    return (
        '<h2>Performance: before vs after dashml</h2>'
        f'<div class="metrics">{col("Before (naive)", before)}'
        f'{col("After (dashml-cleaned)", after)}</div>'
        f"{delta_html}"
    )
...
def _render_html(
    ...
    *,
    ...
    meta_bits.append(f"dashml v{__version__}")
    ...
    return (
        ...
        f"<h2>Findings</h2>"
        f"{findings_html}"
        f"<footer>Generated by dashml. One call: <code>dashml.check(X_train, y_train, X_test, y_test)</code></footer>"
        f"</body></html>"
    )
...
def _render_pdf(
    ...
    *,
    ...
    meta.append(f"dashml v{__version__}")
    ...
    if metrics_before or metrics_after:
        pdf.set_text_color(*_PDF_RGB["fg"])
        pdf.set_font("Helvetica", "B", 11)
        pdf.cell(0, 6, "Performance: before vs after dashml", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(1)
        ...
        for ci, (heading, data) in enumerate(
            [("Before (naive)", metrics_before), ("After (dashml-cleaned)", metrics_after)]
        ):
    ...
    pdf.set_text_color(*_PDF_RGB["muted"])
    pdf.cell(0, 4, _pdf_safe(
        "Generated by dashml. One call: dashml.check(X_train, y_train, X_test, y_test)"
    ), align="C")
