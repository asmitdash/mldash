...
def test_clean_dataset_has_no_critical():
    ...
    report = dash_mlguard.check(Xtr, ytr, Xte, yte)
...
def test_accepts_numpy_arrays():
    X, y = _clean_classification()
    Xtr, ytr, Xte, yte = _split(X, y)
    report = dash_mlguard.check(Xtr.to_numpy(), ytr.to_numpy(), Xte.to_numpy(), yte.to_numpy())
    assert isinstance(report, dash_mlguard.Report)
...
def test_tl011_temporal_overlap_critical():
    ...
    report = dash_mlguard.check(Xtr, ytr, Xte, yte, time_col="ts")
...
def test_tl012_group_overlap_critical():
    ...
    report = dash_mlguard.check(Xtr, ytr, Xte, yte, group_key="user_id")
...
def test_tl013_scaler_fit_on_full_data_detected():
    ...
    report = dash_mlguard.audit_pipeline(pipe, X, y)
    # StandardScaler fit on full vs train will produce different scaling stats
    # for X_test transforms. dash_mlguard should detect this as critical TL013.
...
def test_to_pdf_raises_clear_error_without_fpdf2(monkeypatch):
    ...
    Xtr, ytr, Xte, yte = _split(X, y)
    report = dash_mlguard.check(Xtr, ytr, Xte, yte)
    with pytest.raises(ImportError, match=r"pip install dash-mlguard\[pdf\]"):
...
def test_to_html_includes_new_codes():
    ...
    report = dash_mlguard.check(Xtr, ytr, Xte, yte, time_col="ts", group_key="user_id")
